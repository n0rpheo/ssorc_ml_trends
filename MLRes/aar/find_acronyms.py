...
for abstract_id, row in infoDF.iterrows():
    ...
    if len(acronym_dictionary) > 2000000:
        break

feasible_acronyms = dict()
infeasible_acronyms = dict()
for acronym in acronym_dictionary:
    feasible_definitions = [def_entry for def_entry in acronym_dictionary[acronym] if def_entry['string'].lower() in entities]
    if len(feasible_definitions) > 0:
        feasible_acronyms[acronym] = feasible_definitions
    else:
        infeasible_acronyms[acronym] = acronym_dictionary[acronym]

feasible_ad = dict()
for acronym in feasible_acronyms:
    definitions = [dd['string'] for dd in feasible_acronyms[acronym]]
    ...
    for def_dic in acronym_dictionary[acronym]:
        ...
        if n_def >= 0.5*def_avg:
            if definition not in def_set:
                def_set.add(definition)
                def_list.append(def_dic)

    if len(def_set) > 0:
        feasible_ad[acronym] = def_list
        print(f"{acronym}: {def_set}")

print()
print("Infeasible:")
infeasible_ad = dict()
for acronym in infeasible_acronyms:
    definitions = [dd['string'] for dd in infeasible_acronyms[acronym]]
    n_defs = len(definitions)
    n_unique_defs = len(set(definitions))
